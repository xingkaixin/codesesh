...
本版本将 CodeSesh 后端切换为内嵌 Web UI 的 Rust 原生程序，使模型价格更新不再阻塞会话加载，并减少启动快照的解码工作。 (#636, #643, #644, #645)

### 新功能

- 使用 Rust 原生 CLI 与服务替换 Node.js 后端，保留全部 13 个 Agent 集成、会话搜索与回放、Dashboard、书签、别名及实时更新。 (#636)
- 新增内嵌 Web 资源的独立原生发行包。npm 启动器选择对应平台包，并转发参数、信号与退出状态。 (#636)

### 性能

- 从缓存恢复会话元数据，分批扫描历史，通过持久化 checkpoint 和增量来源指纹复用已完成的工作。 (#636)
- 模型价格变化时，使用缓存用量重新计算估算费用，无需重新读取来源会话；无关模型的价格变化不再使会话扫描失效。 (#643, #644)
- 在后台刷新模型价格，下载缓慢或失败不再阻塞服务启动与会话加载。 (#644)
- 启动快照解码跳过未使用的元数据，并增加启动阶段耗时记录，便于诊断。 (#645)

### 兼容性

- 原生目标为 macOS arm64/x64、Linux x64 GNU（glibc 2.35+）与 Windows x64。独立可执行文件不依赖 Node.js；npm 启动器需要 Node.js 22+。暂不支持更旧 glibc、musl 或 Linux arm64。 (#636)
- 保留本地缓存与用户状态迁移支持。旧 Node 后端已移除，锁定的参考包仅用于兼容性测试。 (#636)

### 构建

- 在 CI 与 tag 触发的发布工作流中加入原生制品打包、内嵌 Web 验证、平台安装 smoke 与制品集合检查。 (#636)
- 升级 HMAC、SHA-2、Base64、Zstandard、React Query 及开发依赖。 (#637, #639, #640, #641, #642)

## [1.0.12] - 2026-09-24

本版本在保留 V1 支持的基础上新增 OpenCode V2 会话历史，采用会话用量避免重复计算 fork 复制的历史，在源数据库路径变化后刷新缓存，并升级 workspace 依赖。 (#631, #632, #633, #634)

### 新功能

- 基于 `2.0.15` 数据结构支持 OpenCode V2 历史，包含按序消息回放、思考、工具结果、附件引用、子会话与上下文压缩记录。Shell 命令和编辑差异沿用现有工具视图，恢复命令仍为 `opencode -s <sessionId>`。 (#633)
- 支持通过 `OPENCODE_DB` 指定 OpenCode 数据库，相对路径基于 `XDG_DATA_HOME/opencode`，默认目录为 `~/.local/share/opencode`。数据库与 WAL 变化会刷新会话详情和搜索，包含消息修改与删除。 (#633)
- OpenCode V2 用量与费用采用会话累计值，避免重复计入 fork 复制的历史或子会话用量。保留明确的零费用；消息用量无法可靠对应会话总量时，不展示模型分摊。 (#633)

### 问题修复

- 源数据库路径变化后刷新数据库会话缓存，即使所选数据库早于上次扫描也会重新读取，使缓存历史与当前数据源保持一致。 (#634)

### 兼容性

- 保留 OpenCode V1 支持，只读访问 V2 数据库。V1 到 V2 迁移未完成或数据无法读取时，报告扫描失败并保留已有缓存。V2 兼容以 `2.0.15` 数据结构为基线，通过构造的 SQLite 样本验证，暂不支持旧开发版数据结构。 (#633)

### 构建

- 升级 React Query、Astro、Hugeicons 与 Node.js 类型定义。 (#631)
- 升级 Turbo、ESLint、Astro lint 与格式化插件、Prettier 及 shadcn lint 规则。 (#632)

## [1.0.11] - 2026-09-21

本版本修复按 Ctrl+C 退出、取消后台任务时误报会话刷新或历史补扫失败的问题。

### 问题修复

- 将扫描与搜索索引 Worker 的正常关闭识别为取消操作，避免误报 `Session refresh failed`、`Backfill failed` 或发布失败。正在执行及排队的任务仍会结束，已有会话快照保持不变，真正的失败仍正常报告。

## [1.0.10] - 2026-09-21

本版本新增 MiniMax Code 会话历史支持，包含会话树、思考、工具详情、用量统计与恢复命令，并缩小产品站移动端首屏插图的高度。 (#627, #628)

### 新功能

- 支持读取 MiniMax Code CLI 0.4.12 的 `v2/sqlite/runtime-state.sqlite`，包含父子会话、消息回放、思考、工具详情、模型用量与费用。恢复命令使用 `mcode --session <sessionId>`。 (#627)
- 默认依次查找 `~/.minimax`、`~/.minimax-code`，选择首个存在数据库的目录；自定义数据目录中，`MINIMAX_DATA_DIR` 优先于 `MAVIS_DATA_DIR`。刷新会识别消息修改和删除，不限于新增消息。 (#627)

### 问题修复

- 检测 MiniMax Code 通过持续打开的文件句柄写入数据库与 WAL 的变化，使目录监听遗漏写入时仍能自动刷新。 (#627)
- 缩小产品站移动端首屏插图的高度，为页面内容留出更多空间。 (#628)

### 兼容性

- MiniMax Code 支持以 CLI 0.4.12 为基线。暂不支持旧 ledger 布局，Desktop 兼容性未验证，媒体仅保留可用引用。 (#627)

## [1.0.9] - 2026-09-19

本版本新增 DeepChat 与 Cherry Studio 会话支持，改善会话阅读布局与小额费用显示，清理数据目录切换后残留的 Claude Code 缓存记录，并新增英文、中文、日文会话历史指南。 (#619, #622, #623, #624, #625)

### 新功能

- 支持从 DeepChat 新版未加密的 `app_db/agent.db` 读取原生及 ACP 会话，包含消息回放、用量统计与实时更新。可通过 `DEEPCHAT_USER_DATA_DIR` 指定数据目录。 (#622)
- 支持从 Cherry Studio 2.x 的 `Data/cherrystudio.sqlite` 读取 Agent 会话及普通助手聊天的当前分支，保留 Agent 工作目录与已存储的消息用量。可通过 `CHERRYSTUDIO_USER_DATA_DIR` 指定数据目录。 (#624)

### 问题修复

- 在刷新和重新加载时移除当前数据目录之外的 Claude Code 缓存会话，不删除原始会话文件。 (#623)
- 改善密集会话的阅读布局：增加日期分隔，允许消息元信息与工具名称换行，在较窄屏幕使用筛选抽屉，并同步产品站演示的响应式布局。 (#619)
- 将不足一美分的正数费用显示为 `<$0.01`，费用图表刻度保留小数，不再取整为美元。 (#619)

### 兼容性

- DeepChat 暂不支持旧版 `chat.db` 与 SQLCipher 加密库。Cherry Studio 仅支持 2.x，暂不支持旧版 `agents.db` 与独立子 Agent 会话树。两项集成均不提供恢复会话命令，也不跨 Agent 去重。 (#622, #624)

### 构建

- 升级 workspace 依赖、pnpm 至 12.4.2，以及 `pnpm/action-setup` 至 6.1.0。 (#620, #621)
- 对 Web 启用主题颜色与类名 lint 检查。 (#618)

### 文档

- 新增英文、中文、日文指南，说明如何定位、搜索与回放本地 Claude Code 和 Codex 历史，排查会话缺失，并解释存储范围。 (#625)

## [1.0.8] - 2026-09-13

本版本在概览与项目 Dashboard 中新增模型 Token 占比，使模型用量统计与所选日期范围一致；统一导航悬停反馈，更新产品站天体视觉，并优化页面资源加载。 (#610, #611, #613, #614, #616)

### 新功能

- 在概览与项目 Dashboard 中展示各模型的 Token 总量和百分比，包含缓存 Token，支持指针与键盘交互。 (#616)
- 为应用侧边栏、分段控件和产品站导航加入一致的动态悬停反馈。 (#611)
- 更新产品站交互式地球及其他天体的表面纹理与材质。 (#610)

### 性能

- 按需加载 Agent 介绍页面，减少 Web 应用首屏加载工作。 (#614)
- 为带内容指纹的产品站资源启用长期缓存，预加载首页主视觉，并避免重复加载 Cloudflare 统计脚本。 (#613)

### 问题修复

- 模型 Token 聚合遵循 Dashboard 所选日期范围，不再计入完整会话中范围外的用量。 (#616)

### 构建

- 升级 workspace 依赖，并让 workspace 安装过程能够使用 node-gyp。 (#614)
- 引入经过审查的 anti-slop lint 规则，并调整源码与测试数据以符合规则。 (#612)
- 清除产品站剪贴板 API 的弃用提示，保留复制回退逻辑。 (#615)

## [1.0.7] - 2026-09-08

本版本在 Overview 概览面板中上线活跃时段分布图（Active Hours），按时段直观展示全天的 AI 交互节奏与活跃频次；加固了实时会话更新与断线重连后的状态恢复机制；使加载失败的会话详情保持可重试；在重叠更新后及时刷新聚合指标；在实时同步中完整保留会话别名并适时失效陈旧视图；在启动时充分利用用户活跃度数据库索引加速计算；并升级 pnpm 至 12.3.4。

### 新功能

- 在 Overview 概览面板新增活跃时段分布图（Active Hours），按一日时段分组，并结合活动频次气泡展示全天的交互分布。 (#598, #603)

### 性能

- 在启动扫描时充分利用用户活跃度数据库索引，加速初始仪表盘指标计算。 (#604)

### 问题修复

- 完善实时会话更新异常与 SSE 重连后的状态恢复机制，保障长连接实时同步的稳定可靠。 (#601, #607)
- 保持不可用会话详情的可重试状态，使临时读取失败无需刷新页面即可重新拉取。 (#606)
- 在发生重叠更新后主动刷新概览聚合指标，防止计数指标陈旧。 (#605)
- 在实时更新中完整保留自定义会话别名，并适时失效共享别名视图，防止别名展示陈旧。 (#599, #600)

### 构建

- 升级 pnpm 至 12.3.4。 (#597)

## [1.0.6] - 2026-09-05

本版本为 Web 产品界面提供完整的英文、简体中文与日文三语本地化支持，支持持久化语言偏好；从 models.dev 获取并缓存最新模型定价以保证成本估算准确；全新改版产品落地页并上线三语产品更新日志；大幅优化全量分析数据读取与会话树遍历性能，忽略 Cursor 共享内存变更以减少无效刷新；加固后台静默刷新、图表 Tooltip、分页快照保留及书签实时同步。

### 新功能

- Web 产品界面完整支持简体中文、英文和日文三语国际化，默认跟随浏览器语言，支持持久化偏好设置，并本地化工具回退输出与会话摘要标签。 (#588)
- 接入 models.dev 模型定价数据并在扫描前自动刷新缓存，提供更及时的模型成本估算。 (#581)
- 全面改版产品落地页，引入交互式 WebGL 会话地球与模块艺术背景，并正式上线中英日三语产品更新日志。 (#568, #569, #572)

### 性能

- 为消息用量查询建立数据库索引，并在 API 路由间共享全时间范围成本事实，大幅加快全历史分析接口的响应速度。 (#586)
- 忽略 Cursor 共享内存文件监听事件，跳过无变更的全量扫描发布，消除冗余的会话刷新开销。 (#587)
- 跳过不必要的会话窗口树遍历，并在树遍历过程中对共享祖先节点仅访问一次。 (#584, #585)
- 改用边界前缀匹配解析模型定价，加快模型价格解析效率。 (#583)

### 问题修复

- 修复 Dashboard 图表 Tooltip：将每日用量 Tooltip 限制在图表可视范围内，并为 Agent 分布图补充 Tooltip 展示。 (#595)
- 抑制不必要的后台刷新通知，使后台自动同步保持安静。 (#582)
- 在分页请求间保留快照一致性，避免翻页时数据产生视图漂移。 (#580)
- 在实时同步时刷新书签视图，并保留已确认的书签写入，防止并发或时钟错位导致丢失。 (#577, #579)
- 显式上报书签存储读取失败，避免错误被静默吞掉。 (#578)

### 重构

- 清理废弃代码与多余封装：移除未使用的 UI 工具函数、路由解码包装、文件分类包装、扫描进度格式化工具、Codex 文本处理及路径提取工具。 (#589, #590, #591, #592, #593, #594)

### 构建

- 升级 pnpm 至 11.25.0，升级 Vitest 至 5。 (#575, #576)
- 升级生产依赖 `hono` 至 4.13.5、`@tanstack/react-query` 至 5.102.8、`react-router-dom` 至 7.18.3、`astro` 至 7.2.9、`vite` 至 8.2.2 与 `open` 至 11.0.2。 (#571)
- 升级开发依赖 `oxfmt` 至 0.65.0、`oxlint` 至 1.80.0、`turbo` 至 2.10.12 与 `typescript-eslint` 至 8.68.0。 (#573)
- 升级 GitHub Action `softprops/action-gh-release` 至 3.0.3。 (#570)

### 文档

- 刷新 Agent 指南与开发规范文档。 (#574)

## [1.0.5] - 2026-08-31

本版本支持从操作菜单一键将完整会话复制为 Markdown，产品落地页接入 Umami 统计；刷新时避免全量重建全文搜索索引，冷会话详情移入后台 Worker 异步加载，加固本地诊断日志与请求关联；并大幅提升 Web 端的实时会话与项目状态一致性。

### 新功能

- 支持从 Web 会话操作菜单和侧边栏将完整会话一键复制为 Markdown。 (#547)
- 产品落地页接入 Umami 统计分析。 (#546)

### 性能

- 增量刷新仅更新发生变更的文档，避免全量重建 FTS 搜索索引，并保持发布进度计数准确。 (#566)
- 将冷会话详情加载移入后台独立 Worker，避免阻塞主流程。 (#540)
- 搜索索引维护调度器中复用会话查找结果，消除冗余查询。 (#544)

### 诊断与日志

- 加固本地诊断日志系统：限制内存与磁盘诊断大小、进程退出前排空日志、支持请求与诊断事件关联，并在 Worker 中保留上下文与排空日志。 (#557)

### 问题修复

- 修复 Claude Code 适配器中子会话丢失父级引用的问题。 (#565)
- 在并发更新时协同刷新活跃项目查询，并将恢复限制在活动时间窗内。 (#564)
- 从单一快照中读取缓存的会话详情，避免并发读取导致的数据不一致。 (#563)
- 修复在过滤条件下新会话消息偶发不可见的问题，确保新会话内容正常展示。 (#562)
- 在搜索、会话过滤及文件活动查询中完整保留实例作用域（query scope）。 (#561)
- 在跨代刷新中保持定价快照 identity 一致。 (#560)
- 对齐并记录 Claude Code 最终请求的 Token 与用量统计，即使无可见内容也保留用量数据。 (#559)
- 仅在持久化写入成功后发布刷新状态，避免实时状态与存储脱节。 (#548)
- 统一 CLI 与 Web 界面中日历天（Calendar Day）时间范围语义与边界计算。 (#549)
- 在重新加载及初始获取期间保留实时更新事件，防止状态被覆盖或丢失。 (#541, #550)
- 在 Web UI 中显式暴露不完整的会话加载状态。 (#542)
- 收到实时事件时及时刷新当前活跃的 Dashboard 作用域。 (#543)
- 解耦会话就绪状态与统计数据计算，避免聚合指标延迟阻塞会话界面的可用性。 (#545)

### 重构

- 按资源领域拆分 CLI API 处理器（会话、Dashboard、Agent 目录、搜索、书签与客户端日志）。 (#552)
- 统一 Agent 基类中的会话源解析逻辑，移除各子类的重复实现。 (#555)
- 在 discovery 扫描中集中管理刷新提交（refresh commits）规划。 (#553)
- 拆分 Web 壳层渲染职责，按路由作用域隔离 Model 与状态。 (#551, #554)
- 将 Web API 客户端与访问上下文和 telemetry 绑定。 (#556)

### 构建

- 移除 Web 端未使用的 PostCSS 依赖。 (#558)
- 升级生产依赖 `@hugeicons/react` 至 1.1.10、`@tanstack/react-query` 至 5.102.1。 (#538)
- 升级开发依赖 `eslint` 至 10.9.0。 (#539)

## [1.0.4] - 2026-08-24

本版本新增日语产品落地页，为环回 API 加入认证并收紧可信代理部署约束；扫描发布与缓存迁移会保留最后一次正确状态，同时减少大规模历史下的索引与渲染工作。本版本也收敛了 core 运行时与 contract API；使用 `@codesesh/core` 的调用方应检查下方兼容性说明。

### 新功能

- 新增完整的日语产品落地页，并集中管理多语言落地页内容。 (#485)
- 暴露扫描流水线耗时与发布阶段，使长时间刷新能够报告有意义的进度。 (#507)

### 安全

- 为环回 API 请求加入认证，并覆盖打包制品的冒烟检查。 (#437)
- 将 `--trust-proxy` 限制为环回 `--host` 与 HTTPS `--public-url` 的组合，防止可信代理后的服务被不安全地直接暴露。 (#438)
- 会话头在进入 browser-safe contract 前统一清理。 (#515)

### 问题修复

- Agent 不可用、变更检查失败、部分回填失败、I/O 失败或发布失败时保留最后一次已提交的扫描状态与刷新基线；刷新检查仅在发布成功后提交。 (#426, #427, #428, #433, #460, #462, #473, #477, #496, #508, #528, #533)
- 让缓存 schema 变更显式且失败安全；移除过时 SQLite 表时不再备份，统一会话行删除，并将临时发布载荷与持久数据库分离。 (#467, #492, #495, #497, #517, #535, #536)
- 修正 Codex Token 统计、Grok 消息分组、Kimi Code 会话头扫描与文件操作分类。 (#461, #509, #510, #512)
- Project Identity 变化后重新索引会话，统一 Dashboard 日期解析，并隔离并发定价缺失捕获的上下文。 (#475, #516, #529)
- 在并发或尾随更新后保持 Web 聚合、项目、别名与书签一致；复用 Dashboard 查询数据，并暴露资源和项目重新加载失败。 (#434, #435, #436, #493, #531, #532)
- 单次 `--json` 路径会在退出前关闭 Worker 与 SQLite 连接。 (#466)

### 性能

- 复用已解析的 Agent 元数据与物化会话数据，简化搜索新鲜度检查，并新增覆盖索引，使未变化的历史不再重读转录正文。 (#468, #507)
- 限制 Project Identity 工作量、SSE 连接数、项目分页、时间线渲染与大内容渲染；同时索引子会话查找并释放非活动投影。 (#439, #440, #443, #444, #445, #446)
- 批量失效会话详情查询，并直接从活动时间窗推导会话请求，减少 Web 端重复查询与刷新。 (#520, #526)
- 将发布载荷暂存在 SQLite 临时 schema 中，使中断的发布无法留下数 GB 的持久副本。 (#467)

### 兼容性

- `--trust-proxy` 现在要求环回 `--host` 与 HTTPS `--public-url`。 (#438)
- `@codesesh/core` 包根入口仅保留一次性扫描能力，原先聚合所有运行时能力的入口已替换为按领域划分的 runtime 子路径。 (#491, #534)
- 会话身份仅通过结构化 reference 暴露；`SessionsUpdatedEvent` 改用 `newSessionRefs`；文件系统数据源改由 `sessionSourceAccess.synchronize` 同步。 (#494, #519, #521)
- `loadCachedSessions` 已替换为返回显式结果的 `readCachedSessions`，调用方必须处理该结果。 (#496)

### 重构

- 将 Agent 记录转换、缓存 schema、搜索索引、扫描规划、同步收尾、Web 会话状态、时间线行为与产品演示场景拆分为职责集中的模块。 (#463, #464, #469, #470, #471, #472, #476, #478, #479, #480, #481, #483, #484, #498, #499, #500, #501, #503, #504, #506, #522, #524, #525, #527, #530)
- 集中管理 Agent 目录、会话 reference key、smart tag 策略与文件数据源能力，并移除生产代码的循环依赖。 (#457, #458, #459, #482, #502, #503, #518)

### 构建

- 升级生产与开发依赖，集中共享依赖版本，维持 Node 22 类型契约，并在 Node 22.0.0 上验证 CLI。 (#448, #449, #450, #451, #452, #453, #487, #489, #490)

### 文档

- 修正文档中的仓库路径，并更新实时运行时、扫描、缓存与 SQLite 架构说明。 (#455, #456)
