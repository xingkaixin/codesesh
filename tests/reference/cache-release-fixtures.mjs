// Frozen from packages/core/src/discovery/__tests__/migration-fixtures.ts
// Reference commit: a545f543a554421b0576058c701ef2ac4190d62e
// Source SHA-256: 64a787e979bb6d56eb8cc0a4b85a0c2e899e40edab78181cc6aa8453ce0938ef
// Only TypeScript syntax and the session-reference import were replaced; fixture SQL is unchanged.

function formatSessionReference(reference) {
  return `${reference.agentName}/${reference.sessionId}`;
}
...
export function hasStructuredMessages(fixture) {
  return fixture.version >= 8;
}

export function expectedBackupCount(fixture) {
  return [6, 15, 30].filter((version) => version > fixture.version).length;
}

export function createReleaseCacheFixture(db, fixture, seed) {
...
function createLegacySchema(db, version) {
...
function createNormalizedSchema(db, version) {
...
function createNormalizedCacheTables(db, version) {
  db.exec(`
    CREATE TABLE cache_meta (
      key TEXT PRIMARY KEY,
      value TEXT NOT NULL
...
function createNormalizedSessionTables(db, version) {
  const parentColumns =
    version >= 18
      ? `
...
function createNormalizedFileActivityTables(db, version) {
  db.exec(`
...
function createNormalizedSearchTables(db, version) {
  if (version >= 17) {
    db.exec(`
      CREATE TABLE session_documents (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
...
function createNormalizedProjectTables(db, version) {
  const parentFilter =
...
function seedSharedRows(db, version, seed) {
...
function seedLegacyRows(db, version, seed) {
...
function seedNormalizedRows(db, version, seed) {
  const session = seed.session;
...
function seedNormalizedSearchDocument(db, version, seed) {
  const session = seed.session;
  if (version >= 17) {
    db.prepare(
      `
...
function seedProjectSession(db, seed) {
  const session = seed.session;
  db.prepare(
    `
