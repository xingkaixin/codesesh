...
function registrationName(registration: (typeof AGENT_CATALOG)[number]): string {
  return registration.name;
}

describe("agent registration completeness", () => {
  it("requires every catalog entry to declare resume support and its tool strategy", () => {
    const incomplete = AGENT_CATALOG.filter((registration) => {
  ...
  it("provides every registered icon to the app and product site", () => {
    const registeredIcons = [
      ...new Set(
        AGENT_CATALOG.map((registration) => {
