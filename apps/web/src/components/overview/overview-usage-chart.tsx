...
function bucketSummary(bucket: DashboardDailyBucket): string {
  return t(
    ...
    [
      ...
      formatCompact(bucketTokens(bucket)),
      formatUsd(bucket.cost),
      formatInt(bucket.sessions),
      formatInt(bucket.messages),
      formatCompact(bucket.input),
      formatCompact(bucket.output),
      formatCompact(bucket.cache_read),
  ...
  return (
    <>
      <div className="mt-[14px] flex items-baseline justify-between border-t border-dashed border-[var(--console-border)] pt-[11px]">
        <span className="console-mono text-[10.5px] text-[var(--console-text)]">
          {t("Daily cost")}
        </span>
        <span className="console-mono text-[10px] text-[var(--console-muted)]">
          {t("Peak")} {formatUsd(peak)} {t("· Avg")} {formatUsd(total / daily.length)}{" "}
          {t("· Total")} {formatUsd(total)}
        </span>
      </div>
      <TileAreaPlot
        values={costs}
...
function DayTooltip({
  ...
  return (
    ...
    >
      ...
      <div>
        {formatInt(bucket.sessions)} {t("sessions ·")} {formatInt(bucket.messages)} {t("messages")}
      </div>
      <div className="text-[var(--console-muted)]">
        {t("In")} {formatCompact(bucket.input)} {t("· Out")} {formatCompact(bucket.output)}{" "}
...
export function OverviewUsageChart({ daily }: { daily: DashboardDailyBucket[] }) {
  ...
  return (
    <Panel role="region" aria-label={t("Daily usage")} className="p-4">
      <PanelHeader
        title={t("Daily usage")}
        action={
      ...
      <p className="console-mono mt-1 text-[10.5px] text-[var(--console-muted)]">
        {t("Stacked bars = token mix · Area below = daily cost")}
        {range}
      </p>
      ...
      <table className="sr-only" aria-label={t("Daily usage data")}>
        <thead>
          <tr>
            <th>{t("Date")}</th>
            <th>{t("Sessions")}</th>
