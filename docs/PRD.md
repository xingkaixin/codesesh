...
> - 支持的 Agent 清单：`packages/contract/src/agent-catalog.ts`
...
- **agent-dump** (Python)：从本地文件系统发现并导出 5 种 Coding Agent（Claude Code、Codex、OpenCode、Cursor、Kimi）的会话记录，统一为 JSON 格式（立项时的范围；当前支持的 Agent 以 `packages/contract/src/agent-catalog.ts` 为准）
