...
- **Claude Code 恢复命令复制** —— 在 Claude Code 会话详情中复制兼容 worktree 的 `claude --resume` 命令
...
- 源码构建需要 Node.js 20.19+ 和 pnpm 10+
...
浏览器会自动打开 `http://localhost:4321`，你的所有会话已就绪。
...
7. **会话详情** —— 点击任意会话查看完整回放，包括 receipt 摘要、用户消息、Assistant 回复、工具调用、推理步骤、模型标签、文件活动和 Claude Code 恢复命令复制
