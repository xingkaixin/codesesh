...
- 2026-08-14：更正 P1-6 的当前态描述。schema 24 已删除 `messages_fts` 与其触发器；当前实现先用 `session_documents_fts` 召回会话，再在最多 50 个候选会话中扫描消息以定位首条精确命中。CS-250 以 `bench:message-search` 重新评估 message-level 索引的查询收益、写入成本和存储增量，不将上述历史实现误述为现状。
- 2026-08-14：完成 CS-250 的成本复核。固定 50 个候选会话 × 2,000 条消息、查询词分散在不同消息的最坏夹具中，trigram FTS 将精确谓词从 100,000 次降至 0，但重复本地运行的中位查询时间仍由约 26–27ms 变为约 28–29ms；全量写入约慢 17 倍，单会话定向更新约慢 10 倍，SQLite 文件增加 20,000,768 bytes。收益不足以覆盖写入与体积成本，因此保留有界候选扫描，并记录运行时投影指标；不以 SQL 全扫替代方案伪装为索引优化。
